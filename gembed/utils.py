...
def binary_crossentropy(preds, labels):
    return np.mean(-labels*np.log(preds+0.00001) - (1-labels)*np.log(1-(preds-0.00001)))
...
def evaluate_preds(preds, labels):
    ...
    for prediction, y in zip(preds, labels):
        split_loss.append(categorical_crossentropy(prediction, y))
        split_acc.append(accuracy(prediction, y))

    return split_loss, split_acc


def evaluate_preds_sigmoid(preds, labels):

    preds = np.array(preds)
    labels = np.array(labels)
    split_loss = binary_crossentropy(preds, labels)
    split_acc = two_class_accuracy(preds, labels)
